...
export default {
  name: 'JobInfo',
  components: { Pagination, JsonEditor, ShellEditor, PythonEditor, Cron },
  directives: { waves },
  filters: {
    statusFilter(status) {
      const statusMap = {
  ...
  methods: {
    ...
    handlerDelete(row) {
      this.$confirm('⚠️确定删除吗？', row.jobDesc, {
    ...
    handlerExecute(row) {
      this.$confirm('确定执行吗？', 'Job: ' + row.jobDesc, {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        ...
        job.triggerJob(param).then(response => {
          this.$alert('执行成功，跳转日志页面', 'Success', {
            type: 'success',
            confirmButtonText: '跳吧',
            callback: action => {
              this.$router.push({ path: '/datax/log/jobLog', query: { jobId: row.id, t: new Date().getTime() }})
            }
          })
        })
      })
